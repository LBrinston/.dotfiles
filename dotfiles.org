...
#+name: WIP 
#+begin_src emacs-lisp :tangle no
(defun get-screenshot-from-dir ()
"Prompts for a a screenshot to be copied from a screenshot directory."
(interactive)
(let (
      ;; pass t to directory-files FULL arg to get full file path
      (screenshot (completing-read "File: " (directory-files "~/vmscreenshots" t)))
      )
  screenshot
  )
)

(message "%s" (get-screenshot-from-dir))

(message "%s" (file-name-base(get-screenshot-from-dir)))

(defun yank-from-dir-to-assets ()
""
(interactive)
(let*(
      ;; Select file
      ;; TODO - Visual interactive file picker would be nice
      (selected-screenshot-path (get-screenshot-from-dir))
      ;; idk why I can't just file-name-base on selected-screenshot-path
      (selected-screenshot-name (file-name-base selected-screenshot-path))
      ;; TODO Prompt for re-name?
      ;; (re-name (read-string "Rename[RET to keep]:"))
      ;; Get dest path
      (assets-dir (get-assets-dir-path))
      ;; Get buffer-name
      (buff-name (file-name-base (buffer-file-name)))
      ;; Construct new path
      (new-path (concat assets-dir "/" buff-name "/" selected-screenshot-name))
  ...
  (message "Selected path: %s\nSelected name: %s\nAssets: %s\nBuff: %s\nNew-Path: %s"
   selected-screenshot-path selected-screenshot-name assets-dir buff-name new-path)
  ;; Check if .assets path exists and create if if not
  ;; Copy file
...
PlantUML can probably be installed by package manager! But watchout if you src_shell{which plantuml} {{{results(=/usr/bin/plantuml=)}}} \larr it might lie to you. If you src_shell{file -b "/usr/bin/plantuml"} you will learn that this is actually a shell script! Which is hiding the location of the jar file:
...
******* Step 2. Wrap the shell script in an elisp function
...
#+begin_src emacs-lisp :comments link :tangle ~/.dotfiles/.config/doom/my-elisp.el
